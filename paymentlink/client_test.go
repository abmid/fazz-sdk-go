...
func TestClient_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx:           context.Background(),
				paymentLinkId: "paymentlink_5362392f1ce3c7dc21ad170acf8f1a09",
				payload: fazz.PaymentLinkUpdatePayload{
					Action: "cancel",
				},
			},
			prepare: func(m helper.Mocks, args args) {
				url := strings.ReplaceAll(fazz.SandboxURL+pathUpdate, ":paymentLinkId", args.paymentLinkId)
				m.Api.EXPECT().
					Req(args.ctx, http.MethodPost, url, nil, args.payload, nil, gomock.Any()).
					DoAndReturn(func(ctx context.Context, method string, url string, param any, body any, header map[string]string, response any) *fazz.Error {
						if err := json.Unmarshal(testWrap.ResJSONByte(pathTest+"res_update_200.json"), response); err != nil {
							panic(err)
						}
			...
			wantRes: &PaymentLinkUpdate{
				Type: "task",
				Attributes: PaymentLinkUpdateAttributes{
					TargetId:   "paymentlink_5362392f1ce3c7dc21ad170acf8f1a09",
					TargetType: "payment_links",
					Action:     "cancel",
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotRes, gotErr := c.Update(tt.args.ctx, tt.args.paymentLinkId, tt.args.payload)
			if !reflect.DeepEqual(gotRes, tt.wantRes) {
