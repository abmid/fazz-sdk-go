...
func TestClient_Payments(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			prepare: func(m helper.Mocks, args args) {
				m.Api.EXPECT().
					Req(args.ctx, http.MethodGet, fazz.SandboxURL+pathPayment, args.params, nil, nil, gomock.Any()).
					DoAndReturn(func(ctx context.Context, method string, url string, param any, body any, header map[string]string, response any) *fazz.Error {
						if err := json.Unmarshal(testWrap.ResJSONByte(pathTest+"res_payments_200.json"), response); err != nil {
							panic(err)
						}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotRes, gotErr := c.Payments(tt.args.ctx, tt.args.params)
			if !reflect.DeepEqual(gotRes, tt.wantRes) {
				t.Errorf("Client.Payments() gotRes = %v, wantRes %v", gotRes, tt.wantRes)
			}
			if !reflect.DeepEqual(gotErr, tt.wantErr) {
				t.Errorf("Client.Payments() gotErr = %v, wantErr %v", gotErr, tt.wantErr)
			}
		})
	}
}
...
func TestClient_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx:       context.Background(),
				paymentId: "contract_b174839e7ddf4d8eb826df7bb3d08a55",
				payload: fazz.PaymentUpdatePayload{
					Action: "receive_payment",
				},
			},
			prepare: func(m helper.Mocks, args args) {
				url := strings.ReplaceAll(fazz.SandboxURL+pathUpdate, ":paymentId", args.paymentId)
				m.Api.EXPECT().
					Req(args.ctx, http.MethodPost, url, nil, args.payload, nil, gomock.Any()).
					DoAndReturn(func(ctx context.Context, method string, url string, param any, body any, header map[string]string, response any) *fazz.Error {
						if err := json.Unmarshal(testWrap.ResJSONByte(pathTest+"res_update_201.json"), response); err != nil {
							panic(err)
						}
			...
			wantRes: &PaymentUpdate{
				...
				Attributes: PaymentUpdateAttributes{
					TargetId:   "contract_b174839e7ddf4d8eb826df7bb3d08a55",
					TargetType: "payment",
					Action:     "receive_payment",
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotRes, gotERr := c.Update(tt.args.ctx, tt.args.paymentId, tt.args.payload)
			if !reflect.DeepEqual(gotRes, tt.wantRes) {
